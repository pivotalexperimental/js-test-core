  ...
  describe SeleniumServerConfiguration do
    describe '#query_string' do
      context "when not passed explicit options" do
        it "defaults selenium_server_start_command to '*firefox' and selenium_host to 'localhost' and selenium_port to 4444 and ignores spec_url" do
      ...
      context "when passed explicit options" do
        attr_reader :configuration, :selenium_server_start_command, :selenium_host, :selenium_port, :spec_url
        before do
          ...
          @configuration = SeleniumServerConfiguration.new(
            :selenium_server_start_command => selenium_server_start_command,
        ...
        it "sets the selenium_server_start_command option" do
          configuration.query_string.should include("selenium_server_start_command=#{CGI.escape(selenium_server_start_command)}")
        end
